...
require_pattern 'ID3TrackMetadataWriter\.write\(patch, to: target\.url\)' 'ID3 carriers must write tags in place on the source file.'
...
reject_pattern 'AtomicMetadataFileReplacer' 'In-place writes must not depend on the atomic swap/rename replacer.'
reject_pattern 'renamex_np|RENAME_SWAP' 'In-place writes must not swap or rename a sibling copy.'
