...
extension DatabaseManager {
    ...
    static func seedDefaultPinnedItems(in db: Database) throws {
        ...
        if pinnedCount == 0 {
            ...
            let recentlyPlayedPlaylist = try Playlist
                .filter(Playlist.Columns.name == DefaultPlaylists.recentlyPlayed)
                .fetchOne(db)
            ...
            if let recentlyPlayed = recentlyPlayedPlaylist {
                let pinnedRecentlyPlayed = PinnedItem(playlist: recentlyPlayed)
                var savedItem = pinnedRecentlyPlayed
                savedItem.sortOrder = 1
                try savedItem.insert(db)
                Logger.info("Pinned default playlist: \(recentlyPlayed.name)")
            }
        }
    }
