...
//  Petrichor
//
//  Handles fetching lyrics from online sources (LRCLIB) and saving them as
//  sidecar LRC files next to the audio file.
//
...
class LyricsManager {
    ...
    func fetchLyrics(for fullTrack: FullTrack) async -> String? {
        ...
        if let lyrics = await fetchFromLRCLIB(fullTrack: fullTrack) {
            persistLyricsSidecar(lyrics, for: fullTrack)
            return lyrics
        }
    ...
    private func persistLyricsSidecar(_ lyrics: String, for fullTrack: FullTrack) {
        do {
            try LyricsSidecarWriter.write(lyrics, forAudioURL: fullTrack.url)
            Logger.info("LyricsManager: Stored lyrics sidecar for '\(fullTrack.title)'")
        } catch {
            Logger.error("LyricsManager: Failed to store lyrics sidecar - \(error.localizedDescription)")
        }
    }
}
