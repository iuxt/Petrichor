...
class ScrobbleManager: ObservableObject {
    ...
    func authenticationURL() -> URL? {
        guard let apiKey = apiKey, !apiKey.isEmpty else {
            ...
            NotificationManager.shared.addMessage(.error, String(appLocalized: "Last.fm API key not configured"))
            return nil
        }
    ...
    func handleAuthCallback(_ url: URL) {
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: false),
            ...
            NotificationManager.shared.addMessage(.error, String(appLocalized: "Last.fm authorization failed: missing token"))
            return
        }
    ...
    private func exchangeToken(_ token: String) async {
        guard let apiKey = apiKey,
            ...
            await MainActor.run {
                NotificationManager.shared.addMessage(.error, String(appLocalized: "Last.fm API credentials not configured"))
            }
            return
        }
            ...
            if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
               ...
               let username = session["name"] as? String {
                ...
                await MainActor.run {
                    ...
                    NotificationManager.shared.addMessage(.info, String(appLocalized: "Connected to Last.fm as \(username)"))
                }
                ...
                await MainActor.run {
                    NotificationManager.shared.addMessage(.error, String(appLocalized: "Last.fm error: \(message)"))
                }
            }
        } catch {
            Logger.error("Session request failed - \(error.localizedDescription)")
            await MainActor.run {
                NotificationManager.shared.addMessage(.error, String(appLocalized: "Failed to connect to Last.fm"))
            }
        }
    }
